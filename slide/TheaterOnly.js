...
import TheaterOnlyView from './TheaterOnlyView'

class TheaterOnly extends React.Component{ 
    state = {
        results :[],
        results1 : [],
        results2 :[]  
  ...
  getDescription = async () => {
      ...
      const recup_data1 = await fetch(`https://api.themoviedb.org/3/movie/now_playing?${api_key}&language=en-FR&page=2&region=FR&language=fr`)
      const recup_data2 = await fetch(`https://api.themoviedb.org/3/movie/now_playing?${api_key}&language=en-FR&page=3&region=FR&language=fr`)
    ...
    render(){
        return(
            <div className="TheaterOnly">
            <div className="box">
            {this.state.results.map((element, id) => 
                    <TheaterOnlyView key={element.id} title = {element.title} 
                        id={element.id}
